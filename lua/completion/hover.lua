  ...
  if vim.fn.pumvisible() == 1 then
    M.focusable_float(ctx.method, function()
      ...
      if winnr ~= nil and api.nvim_win_is_valid(winnr) then
        vim.api.nvim_command(string.format('autocmd CursorMoved, BufHidden, InsertCharPre <buffer> ++once lua pcall(vim.api.nvim_win_close, %d, true)', winnr))
      end
    ...
    if manager.textHover == true and items['selected'] ~= -1 then
      ...
      if  user_data ~= nil and user_data['lsp'] == nil then
        ...
        for _, value in pairs(vim.lsp.buf_get_clients(0)) do
          if value.server_capabilities.hoverProvider then
            has_hover = true
            break
          end
        end
